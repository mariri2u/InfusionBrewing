...
public class BehaviorDispencePotionBucket extends BehaviorDefaultDispenseItem {
	...
	public ItemStack dispenseStack(IBlockSource source, ItemStack itemstack) {
		...
		Item item = DISPENSE_MAGIC_BUCKET ? InfusionBrewing.itemMagicBucket : Items.bucket;
		
		if(block == Blocks.air){
			world.setBlock(x, y, z, InfusionBrewing.blockFluidPotions[((ItemPotionBucket)itemstack.getItem()).getPotionEffect() - 1]);
